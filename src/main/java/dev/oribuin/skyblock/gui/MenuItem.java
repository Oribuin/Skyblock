...
//        }
        ...
        ItemStack item = this.customItem != null
                ? this.customItem
                : ItemConstruct.deserialize(this.config).build(this.placeholders);

        if (item == null) {
            SkyblockPlugin.get().getLogger().warning("Item [" + this.itemPath + "] in the [" + this.config.getName() + "] menu is invalid.");
            return;
        }
