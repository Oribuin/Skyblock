...
class CreateGUI(rosePlugin:RosePlugin) :xyz.oribuin.skyblock.gui.
...
PluginGUI(rosePlugin) {
    ...
    fun openMenu (member:Member){
        ...
        this.worldManager.schematics.forEach {
            (_, schem) ->
                    val item = GuiItem(
                    skyblock.util.ItemBuilder(schem.icon)
            ...
            item.setAction {
                this.manager.makeIsland(member, schem)
            }
            gui.addItem(item)
        }
